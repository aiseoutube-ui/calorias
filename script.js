...
const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzA3XV9_VH7p4N8pjWLi7uhH4xrEwO4G93_TR24tBdKtPzlFUCkIg1UHiHY6a1A4PAi/exec';
//================================================================================
...
// ** CAMBIO: La lógica de cálculo se mueve al backend **
calculatePlanButton.addEventListener('click', async () => {
    const planInputs = {
        ...userProfile,
    ...
    setButtonLoading(calculatePlanButton, true);
    ...
    try {
        ...
        setButtonLoading(calculatePlanButton, false);
    }
});
...
// ** CAMBIO: La construcción del prompt se mueve al backend **
...
getSuggestionButton.addEventListener('click', async () => {
    ...
    try {
        const response = await sendDataToBackend({
            action: 'getSuggestion', // Nueva acción para el backend
            payload: {
                selectedOption,
                userPrompt,
